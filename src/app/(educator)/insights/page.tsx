...
const SEVERITY_CLASS: Record<string, string> = {
  conceptual: "border-[color-mix(in_srgb,var(--color-disputed)_40%,transparent)] bg-disputed-soft text-disputed",
  procedural: "border-[color-mix(in_srgb,var(--color-attention)_40%,transparent)] bg-attention-soft text-attention",
  ...
  return (
    <main className="mx-auto flex max-w-3xl flex-col gap-lg px-6 py-xl">
        ...
        {assessments.map((a: any) => (
          <Link
            ...
            className={`rounded-pill border px-md py-xs text-body-sm ${
              a.id === selectedId
                ? "border-[color-mix(in_srgb,var(--color-primary)_40%,transparent)] bg-primary-soft text-body-strong"
      ...
      {insights.submissionCount === 0 ? (
        ...
        <>
          <section className="glass-card flex flex-col gap-sm p-lg">
            ...
            <div className="flex items-end gap-xxs" style={{ height: 96 }}>
              {insights.scoreHistogram.map((b) => (
                <div key={b.bucket} className="flex flex-1 flex-col items-center gap-xxs">
                  <div
                    className="w-full rounded-t-sm bg-gradient-to-t from-[var(--gradient-accent-1)] to-[var(--gradient-accent-3)]"
                    style={{ height: `${(b.count / maxHistogramCount) * 80}px`, opacity: b.count ? 1 : 0.15 }}
                  />
                  <span className="font-mono text-caption text-muted-soft">{b.bucket * 10}</span>
                </div>
              ))}
            </div>
                ...
                {insights.criteriaBreakdown.slice(0, 10).map((c, i) => (
                  <div key={i} className="flex items-center gap-sm">
                    ...
                    <div className="h-2 flex-1 rounded-pill bg-surface-soft">
                      <div
                        className="h-2 rounded-pill bg-gradient-to-r from-[var(--gradient-accent-3)] to-[var(--color-primary-active)]"
                        style={{ width: `${Math.round(c.avgPct * 100)}%` }}
                      />
                    </div>
                    <span className="w-12 shrink-0 text-right font-mono text-data-sm tabular-nums text-muted">
