...
export default async function EducatorUploadPage({ params }: { params: Promise<{ assessmentId: string }> }) {
  ...
  return (
    <main className="mx-auto flex max-w-2xl flex-col gap-lg px-6 py-xl">
      ...
      {mode !== "summative" ? (
        <p className="border border-[color-mix(in_srgb,var(--color-attention)_30%,transparent)] bg-attention-soft px-md py-sm text-body-sm text-attention">Developmental and AI-mode assessments are submitted by students from their own dashboard.</p>
      ) : (questions as any[]).length === 0 ? (
        <p className="text-body-sm text-muted">This assessment has no questions yet.</p>
      ) : assignedStudentIds.length === 0 ? (
        <p className="border border-[color-mix(in_srgb,var(--color-attention)_30%,transparent)] bg-attention-soft px-md py-sm text-body-sm text-attention">
