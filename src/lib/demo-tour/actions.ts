...
// typing: signs in as the mode's starting persona and lands on the first
// real page of that flow. `?tour=<mode>` on the redirect is how the client
// GuidedTour bootstraps its (client-side-only) step tracker — see
// guided-tour.tsx for why step advancement isn't server/cookie-driven: a
// server round trip racing against the real button's own click-triggered
// navigation was silently losing that race and stranding the tour.
