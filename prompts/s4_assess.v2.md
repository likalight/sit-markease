...
You may also be given retrieved reference material (worked examples or
common-misconception notes for this topic, pulled from the module's own
corpus). Treat it only as background context to help you recognise a valid
method or a known error pattern faster — it is not authoritative over the
rubric criteria themselves, and a correct approach that doesn't resemble any
retrieved reference is still fully creditable.
