...
// Hand-authored single-read transcriptions for each gold script — see
// eval/gold/images/*.png for the rendered text. One model, one read: no
// second independent vendor read to cross-check against (docs/DECISIONS.md
// — dropped the dual-vendor design; trust now comes from this model's own
// reported confidence/legibility plus S4's symbolic answer check).

const FIXTURES: Record<string, TranscriptionRead> = {
  correct: {
    steps: [
...
async function main() {
  ...
  for (const [goldId, fixture] of Object.entries(FIXTURES)) {
    ...
    aiCache.seed(
      { promptVersion: PROMPT_VERSIONS.s2Read, provider: PRIMARY_PROVIDER, model: PRIMARY_MODEL, system: readSystem, prompt: readPrompt, imageHashes: [imageHash] },
      fixture
    );

    console.log(`  seeded S2 fixture for ${goldId} (key ${key.slice(0, 8)}…)`);
  }
